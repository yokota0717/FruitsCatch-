...
extern int finalScore;  //最終的総スコア
extern int highScore[6];  //ランキング作成用スコア
...
//	DI::VGamePad in1 = DI::GPad_GetState("P1");
	...
	std::vector<ResName> resname
	{
		...
		{"KagoImg", "./data/image/kago.png"},  //かご
		...
		{"CursorImg", "./data/image/cursor.png"},  //カーソル
		{"MoreImg", "./data/image/onemore2.png"},  //リトライ文字
		{"OwariImg", "./data/image/owari2.png"},  //エンド選択肢
	...
	TaskFlag  Update()
	{
		...
		if (timeCnt >= 1860) {
			if (cursor.x == 90) {
				if (in1.B1.down) {  //もう一回選択肢が押されたら
					rtv = Task_HowTofor1;  //次のタスクをHowToへ
				}
			}
			if (cursor.x == 290) {
				if (in1.B1.down) {  //おわり選択肢が押されたら
					//ランキングを更新
					highScore[5] = totalScore;
					bubbleSort(highScore, 6);
					rtv = Task_Endingfor1;  //次のタスクをエンディングへ
				}
			}
		}
    	return rtv;
	...
	void  Render2D()
	{
		...
		if (totalScore < 0) {  //スコアがマイナスになった時の対処
